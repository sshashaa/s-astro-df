...
function [Ancalls, Asoln, Afn, AFnVar, AFnGrad, AFnGradCov, ...
    ...
    = SOWC1(probHandle, probstructHandle, problemRng, solverRng, m_rep)
...
%     trainData = readmatrix('WindDataSPlitTrainwo');
%     trainData(:,1) = [];
...
function [callcount, x_points, func_points, var_points, ptdict] ...
    = ASTRDF_Internal(probHandle, problemRng, solverRng, ...
        minmax, dim, VarBds, xk, budgetmax, type, Delta0, Delta_max, ptdict, PT, BestS, K, trainData, pw, wi_0, sm_strata)
...
%         end
        ...
        [q, Fbar, Fvar, Deltak, calls, ptdict, budgetmax, Y, wi_xk] = Model_Construction(probHandle, xk, Delta, iteration_number, ...
            type, w, mu, beta, calls, solverInternalRng, problemRng, minmax, ptdict, VarBds,lin_quad,budgetmax, PT, BestS, dim, K, wi_0, pw, sm_strata, trainData);
        ...
        while 1
                if ~isempty(Na_p)
                if sum(samplecounts(1:K)>=Na_p) == K
                    Na_p = [];
                    if ~isempty(wi)
                        samp_type = 1;
                    end
                ...
                Nt = sum(samplecounts(1:K));
                if Nt >= K*10 
                    wi = pw(1:K).*sqrt(sig2_bins(1:K))/sum(pw(1:K).*sqrt(sig2_bins(1:K)));
                else
                    wi = [];
                end
                %continue until a good sample size is found
                if isempty(Na_p)
                    Na = Adaptive_Sampling(iteration_number, sig2, Deltak, type, size(x_tilde,2), wi_xk, samplecounts(1:K), K, samp_type, wi, Fb);
                else
                    Na = Na_p;
                end
                if isempty(Na_p) && Nt >= K*10
                    Na_p = Na;
                end
            if sum(samplecounts(1:K) >= Na) == K
                break
            else
                if sum(samplecounts(1:K)) > 40 && calls > budgetmax
                    break
                end
                Nt = Nt + 1;
            end
        for k = 1:K
            ...
            [xi, ~, ~, ~, ~, ~, ~, ~, r_ws] = probHandle([x_tilde sm_strata(k) k], 1, problemRng, samplecounts(k), 1);
...
%             sig2 = (sum(samplecounts)-2)/(sum(samplecounts)-1)*sig2 + sum(samplecounts)*(Fb-F_old)^2; %update variance
            ...
            for tot_var = 1:K
                ...
                if isempty(wi)
                    wi_temp = 1/K*ones(1,K);
                    tau = tau + (sig2_bins(tot_var)*pw(tot_var)^2/wi_temp(tot_var));
                else
                    tau = tau + (sig2_bins(tot_var)*pw(tot_var)^2/wi(tot_var));
                end
            end
            sig2 = temp_sig2;
            end
            if sum(samplecounts(1:K)) == 1
                sig2 = 0;
                tau = 0;
            end
            if sum(samplecounts(1:K)) == Nt
                break
            end
        end
        end
...
function [q, Fbar, Fvar, Deltak, calls, ptdict, budgetmax, Y, wi_xk] = Model_Construction(probHandle, x, Delta, k, type, ...
    w, mu, beta, calls, solverInternalRng, problemRng, minmax, ptdict,VarBds,lin_quad, budgetmax, PT, BestS, dim, K, wi_0, pw, sm_strata, trainData)
        ...
        for i = 1:p %for each point in Y
            ...
            while 1
                ...
                if isempty(Na_p)
                    ...
                    Na = Adaptive_Sampling(k, sig2, Deltak, type, size(Y,2), wi_xk, ks(1:K), K, samp_type, wi, Fb);
                else
                    Na = Na_p;
                end
                if isempty(Na_p) && Nt >= K*10
            ...
            for l = 1:K
                ...
                ks(l) = ks(l) + 1;
                [xi, ~, ~, ~, ~, ~, ~, ~, r_ws] = probHandle([Y(i,:) sm_strata(l) l], 1, problemRng, ks(l), 1);
                xi = -minmax*xi; % Account for minimization/maximization
                problemseed = problemseed + 1; % iid observations  
                calls = calls + 1;
                [~, r_idx] = ismember(r_ws, trainData(:,1));
                F_old = Fb;
...
function numsamples = Adaptive_Sampling(k, sig2, Delta, type, dim, wi_xk, ks, K, samp_type, wi, Fb)
...
%     end
    ...
    if N > sum(ks) && sum(ks) >= 40 && min(ks) > 2
