function [fn, FnVar, FnGrad, FnGradCov, constraint, ConstraintCov, ConstraintGrad, ConstraintGradCov, r_ws] = WINDRASCMSEADD2D(theta, runlength, problemRng, seed, m_rep)
...
if isempty(currData) 
    % turbine data
    currData = readmatrix('OWEZ240R75R2DB.csv');
...
%     currData(:,6:41) = [];
    ...
    rWS = 15;
    % thrust Coefficient
    dat = readmatrix("Default Dataset.csv");
    dat_ws = dat(:,1);
    dat_Ct = dat(:,2);
...
if (runlength <= 0) || (runlength ~= round(runlength)) || (seed <= 0) || (round(seed) ~= seed)
    ...
    if size(theta,2) == 2
        ...
        bin1_test = currData(bin1_test_idx,1:4);
        ...
        bin2_test = currData(bin2_test_idx,1:4);
        ...
        bin3_test = currData(bin3_test_idx,1:4);
        ...
        bin4_test = currData(bin4_test_idx,1:4);
        ...
        test_pred = windCode(test(:,1), test(:,2), theta_i, quant_range_basis, coordinate, turbineDia, Ct, sp, fPower, x1, test(:,4));
        else
        if m_rep >= 10
...
%         post_sample = datasample(testData,test_size);
        ...
        % test data set
        test = post_sample(:,1:4); 
        test_actual = post_sample(:,18:end);
        test_pred = windCode(test(:,1), test(:,2), theta, quant_range_basis, coordinate, turbineDia, Ct, sp, fPower, x1, test(:,4));
    ...
    if cell2mat(theta(3)) == 0
        ...
        test = sample_data(:,1:4);
        test_actual = sample_data(:,18:end);
        test_pred = windCode(test(:,1), test(:,2), theta_o, quant_range_basis, coordinate, turbineDia, Ct, sp, fPower, x1, test(:,4));
...
%%  wind wake model
function windSpeedHat = wake1(windSpeed, windAng, k0, coordinate, turbineDia, Ct_fit, windTI)
...
for i = 1:size(x,1) 
    ...
    def = aa(i)./(1 + 2*k(i).*delX).^2;
    % remove turbines that are not in wake 
    def(delX <= 0) = 0;
    def(delY > (0.5 + k(i).*delX)) = 0;
...
%% wind power model
function powerHat = windCode(xSeq, WDSeq, theta, quant_range_basis, coordinate, turbineDia, Ct, sp, fPower, x1, TISeq)
windSpeedHat = wake1(xSeq, WDSeq, theta, coordinate, turbineDia, Ct, TISeq);
