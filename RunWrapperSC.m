function RunWrapperSC(problemnameArray, solvernameArray, repsAlg)
...
for k1 = 1:length(problemnameArray)
    ...
    for k2 = 1:length(solvernameArray)   
        ...
        parfor j = 1:repsAlg
            ...
            for i = 1:NumRngs
                problemRng{i} = RandStream.create('mrg32k3a', 'NumStreams', (2 + NumRngs)*repsAlg, 'StreamIndices', (2 + NumRngs)*(1 - 1) + 2 + i);
            end
            ...
            [Ancalls_cell{j}, A_cell{j}, AFnMean_cell{j}, AFnVar_cell{j}, ~, ~, ~, ~, ~, ~] = solverHandle(probHandle, probstructHandle, problemRng, solverRng, j);
